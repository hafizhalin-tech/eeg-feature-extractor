...
if uploaded_files:
    ...
    # Mutual information
    X = features_df.drop(columns=["Label"]) if "Label" in features_df.columns else features_df
    y = features_df["Label"]
    ...
    threshold = st.sidebar.slider("Minimum MI to keep feature", min_value=0.0, max_value=mi_max, value=0.0, step=max(mi_max/100.0, 1e-6))
